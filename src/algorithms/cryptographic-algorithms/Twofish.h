...
namespace Sequency
{
    ...
    private:
        ...
        uint8_t ExtKey[288] = { 0 };
        uint8_t* eKey;

        static constexpr int IK0 = 0;    // Initial index for input key
        static constexpr int EK0 = IK0 + 16;
        static constexpr int IK4 = 16 * 16 + 16;  // Initial index for input key
        ...
        void setup(uint8_t* secret)
        {
            uint32_t wk[LINE];
            uint32_t ek[ROUND * 4 + 8];
            int i, j, n = ROUND + 2;
            int cnt = 0;

            memcpy(wk, secret, LINE * 4);
    ...
    public:
        static constexpr int ROUND = 16;
        static constexpr int LINE = 8;

        Twofish(uint8_t* secret) : eKey(ExtKey)
        {
            setup(secret);
        }
        ...
        {
            static uint32_t counter = 0; // Counter to update initial values
            uint32_t ida = 0; // Set the initial value of ida here
            uint32_t idb = 0; // Set the initial value of idb here
            uint32_t k[4] = { 0 }; // Set the initial values of k here

            // Update initial values using the counter
            ida += counter;
            idb += counter;
