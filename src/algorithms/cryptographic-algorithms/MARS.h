...
namespace Sequency
{
    class MARS {
    private:
        std::array<int32, 40> l_key;
        std::array<int32, 4> counter;

        void f_mix(int32& a, int32& b, int32& c, int32& d) 
        {
            int32 r;
            r = rotate_right32(a, 8);
            b ^= s_box[a & 255];
            b += s_box[(r & 255) + 256];
            r = rotate_right32(a, 16);
            a = rotate_right32(a, 24);
            c += s_box[r & 255];
            d ^= s_box[(a & 255) + 256];
        }

        void b_mix(int32& a, int32& b, int32& c, int32& d)
        {
            int32 r;
            r = rotate_left32(a, 8);
            b ^= s_box[(a & 255) + 256];
            c -= s_box[r & 255];
            r = rotate_left32(a, 16);
            a = rotate_left32(a, 24);
            d -= s_box[(r & 255) + 256];
            d ^= s_box[a & 255];
        }

        std::array<int32, 512> s_box = {
    ...
    public:
        MARS(const int32 key[4])
        {
            set_key(key, 128);  // 128-bit key length
        }

        void set_key(const int32 in_key[], const int32 key_len)
        {
            int32 i, j, m, w;
            int32 vk[47] = {
                    0x09d0c479, 0x28c8ffe0, 0x84aa6c39, 0x9dad7287, 0x7dff9be3, 0xd4268361, 0xc96da1d4
            };
            ...
            for (i = j = 0; i < 39; ++i)
            {
                vk[i + 7] = rotate_left32(vk[i] ^ vk[i + 5], 3) ^ in_key[j] ^ i;
                j = (j == m ? 0 : j + 1);
            }
            ...
            {
                for (i = 1; i < 40; ++i)
                    vk[i + 7] = rotate_left32(vk[i + 7] + s_box[vk[i + 6] & 511], 9);
                vk[7] = rotate_left32(vk[7] + s_box[vk[46] & 511], 9);
            }
            ...
            for (i = 5; i < 37; i += 2)
            {
                ...
                m = gen_mask(w);
                if (m)
                    w ^= (rotate_left32(s_box[265 + (l_key[i] & 3)], l_key[i + 3] & 31) & m);
                l_key[i] = w;
            }
        }
        ...
        {
            int32 in_blk[4] = { counter[0], counter[1], counter[2], counter[3] };
            int32 out_blk[4];
            ...
            return (out_blk[0] % (max - min + 1)) + min;
        }

        void encrypt(const int32 in_blk[4], int32 out_blk[4])
        {
            int32 a, b, c, d;
            a = in_blk[0] + l_key[0];
            b = in_blk[1] + l_key[1];
        ...
        void f_ktr(int32& a, int32& b, int32& c, int32& d, int32 i)
        {
            int32 m, r, l;
            m = a + l_key[i];
            a = rotate_left32(a, 13);
            r = a * l_key[i + 1];
            l = s_box[m & 511];
            r = rotate_left32(r, 5);
            c += rotate_left32(m, r);
        ...
        void b_ktr(int32& a, int32& b, int32& c, int32& d, int32 i)
        {
            int32 m, r, l;
            r = a * l_key[i + 1];
            a = rotate_right32(a, 13);
            m = a + l_key[i];
            l = s_box[m & 511];
            r = rotate_left32(r, 5);
            l ^= r;
